...
resource "aws_cloudwatch_log_group" "app" {
  name              = "/ecs/${var.environment}-only1mcp"
  retention_in_days = 7

  tags = {
    Name = "${var.environment}-only1mcp-logs"
  }
}
...
resource "aws_iam_role" "ecs_task" {
  name = "${var.environment}-only1mcp-ecs-task"
...
resource "aws_ecs_task_definition" "app" {
  family                   = "${var.environment}-only1mcp"
  network_mode             = "awsvpc"
...
resource "aws_appautoscaling_policy" "cpu" {
  name               = "${var.environment}-only1mcp-cpu"
...
resource "aws_appautoscaling_policy" "memory" {
  name               = "${var.environment}-only1mcp-memory"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.ecs.resource_id
  scalable_dimension = aws_appautoscaling_target.ecs.scalable_dimension
  service_namespace  = aws_appautoscaling_target.ecs.service_namespace
